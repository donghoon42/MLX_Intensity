...
private:
    ...
    std::vector<sensor_msgs::ImagePtr> intensity_vec;
    std::string bin_file;
        ...
        for(uint64_t i=0; i<total_num_frame; i++){
            if(lidar_ml->get_scene(scene, i)){        
                ...
                if(!scene.intensity_image.empty()){
                    ...
                    intensity_vec.push_back(msg_intensity);
                ...
                std::string progress = "Progress    [" + std::to_string(i) + "/" + std::to_string(total_num_frame) + "]";
                std::cout << "\n\r\033[F" << progress << std::flush;
            }
    ...
    if(loop){
        while(ros::ok()){
            for(int i=0; i<pc_vec.size(); i++){
                ...
                std::string progress = "Loop       [" + std::to_string(i) + "/" + std::to_string(size_of_frmae) + "]";
                std::cout << "\n\r\033[F" << progress << std::flush;
                publisher_pc.publish(pc_vec[i]);
                pub_intensity.publish(intensity_vec[i]);
                std::this_thread::sleep_for(std::chrono::milliseconds(mil_sec));
            }
        }
    }
